# Futures_Dual (grid-trading)

一个基于 Go 的 Binance USDⓈ-M 合约网格策略项目，支持：

- **回测**（`mode: backtest`）
- **测试网实盘**（`mode: testnet`）
- **生产实盘**（`mode: live`）

核心策略为 `FuturesGrid`：支持 `dual/long/short` 三种合约网格模式，带状态持久化、重连对账、断路器和告警能力。
...
- Go 1.21+
- Binance Futures API Key（testnet 或 live）
...
- `grid.contract_mode`（必须显式填写：`dual` / `long` / `short`）
- `backtest.data_path`
...
策略会在 `state_futures/{mode}/{symbol}/{instance_id}` 下维护状态（网格状态、开单快照、运行状态、锁文件）。

---

## 5. 关键配置说明（节选）

- `grid.contract_mode`：`dual` / `long` / `short`
- `grid.ratio`：买网格几何比率（>1）
- `grid.sell_ratio`：卖网格几何比率（>1）
- `grid.levels`：初始化网格层数（dual 两侧都用；long/short 用于单边初始化）
- `grid.shift_levels`：上下移步长（dual 上下移都用；long 仅下移；short 仅上移）
...
- `exchange.position_mode`：`hedge` / `oneway`（`dual` 必须用 `hedge`）
- `exchange.margin_type`：`cross` / `isolated`
- `exchange.leverage`：杠杆倍数（1..125）
...
### Q3: 实盘重启后是否能恢复
会从 `state_futures` 目录加载网格状态与开单快照，并执行 reconcile。
